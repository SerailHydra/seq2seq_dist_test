...
def parse_args():
    ...
    results.add_argument('--save', default='gnmt_wmt16_'+str(os.getpid()),
    ...
    distributed.add_argument('--gpu-rank', default=0, type=int,
                             help='If using distributed parallel for multi-gpu, sets the GPU for the process')
    # profiling
    profile = parser.add_argument_group('profile setup')
    profile.add_argument('--profile-dir', default="./profile", type=str,
                         help="Directory for dumping profiling data")
    profile.add_argument('--profile', action='store_true',
                         help="Profile passed in model")
    profile.add_argument('--num-minibatches', default=20, type=int,
                         help="number of minibatches to profile")
    profile.add_argument('--cupti', action='store_true',
...
def build_criterion(vocab_size, padding_idx, smoothing):
    if smoothing == 0.:
        logging.info('building CrossEntropyLoss')
        ...
        logging.info('building SmoothingLoss (smoothing: {})'.format(smoothing))
