...
class DistributedDataParallel(Module):
    ...
    def __init__(self, module, log_dir=None, message_size=10000000, shared_param=False):
    ...
    def create_hooks(self):
        ...
        def allreduce_params():
            ...
            flat_dist_call(grads, dist.all_reduce, log_dir=self.log_dir, iteration=self.iteration, msg="allreduce_{}".format(self.dist_call_id))
            self.iteration -= 1
            self.dist_call_id += 1
        ...
        def flush_buckets():
            ...
            if(len(grads)>0):
                ...
                with torch.cuda.stream(self.reduction_stream):
                    ...
                    flat_dist_call(grads, dist.all_reduce, log_dir=self.log_dir, iteration=self.iteration, msg="flush_{}".format(self.dist_call_id))
                    self.dist_call_id += 1
    ...
    def comm_ready_buckets(self, param_ind):
        ...
        while grads:
            ...
            with torch.cuda.stream(self.reduction_stream):
                flat_dist_call(bucket, dist.all_reduce, log_dir=self.log_dir, iteration=self.iteration, msg="ready_{}".format(self.dist_call_id))
                self.dist_call_id += 1
